...
public class Stage1 extends JComponent implements Runnable{
    ...
    public void paint(Graphics g) {
        ...
        g.fillRect(pac.x + 30, pac.y - 5, pac.width, pac.height + 10); // Direita
        g.fillRect(pac.x - 30, pac.y - 5, pac.width, pac.height + 10); // Esquerda
        g.setColor(Color.BLACK);
        g.fillRect(pac.x - 5, pac.y + 30, pac.width + 10, pac.height); // Baixo
        g.fillRect(pac.x - 5, pac.y - 30, pac.width+10, pac.height); // Cima
        // Campo de verificação do Pac  */ 
        // Desenha PAC
        pac.draw(g);
        pac.move();
        if (pac.score >= 200 || pac.lives == 0) {
            pac.x = 385;
            pac.y = 315;
            pac.velx = 0;
            pac.vely = 0;
